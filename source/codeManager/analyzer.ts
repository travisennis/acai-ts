...
export async function analyzeCodebase(
  ...
  try {
    ...
    if (allFiles.length > maxFiles) {
      logger.warn(
        `Codebase has too many files (${allFiles.length}), limiting to ${maxFiles} files`,
      );
    ...
    throw new Error(
      `Failed to scan codebase: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  for (const file of allFiles) {
    try {
      ...
      if (stats.size > maxSizePerFile) {
        logger.debug(
          `Skipping file (too large): ${file} (${formatFileSize(stats.size)})`,
        );
        skippedFiles++;
        continue;
      }
      ...
      projectStructure.filesByLanguage[language] =
        (projectStructure.filesByLanguage[language] || 0) + 1;
  ...
  // Log results
  logger.info(
    `Codebase analysis complete: ${processedFiles} files analyzed, ${skippedFiles} files skipped`,
  );
