...
export class Cell {
  ...
  setOptions(options: CellOptions | string | number | boolean | bigint): void {
    ...
    if (
      ["boolean", "number", "bigint", "string"].indexOf(
        typeof processedOptions,
      ) !== -1
    ) {
      processedOptions = { content: `${processedOptions}` } as CellOptions;
    }
    const opts = (processedOptions || {}) as CellOptions;
    this.options = opts;
    const content = opts.content;
    if (
      ["boolean", "number", "bigint", "string"].indexOf(typeof content) !== -1
    ) {
      ...
      throw new Error(
        `Content needs to be a primitive, got: ${typeof content}`,
      );
    }
  ...
  computeLines(tableOptions: TableInstanceOptions): string[] {
    ...
    if (this.fixedWidth && wordWrap) {
      ...
      const { wrapOnWordBoundary: tableWrapOnWordBoundary = true } =
        tableOptions;
      const { wrapOnWordBoundary = tableWrapOnWordBoundary } = this.options;
      return this.wrapLines(
        multiLineWordWrap(fixedWidth, this.content, wrapOnWordBoundary),
      );
    }
  ...
  init(tableOptions: TableInstanceOptions): void {
    ...
    const y = this.y;
    this.widths = (
      tableOptions.colWidths.slice(x, x + this.colSpan) as number[]
    ).filter((w): w is number => w !== null);
    this.heights = (
      tableOptions.rowHeights.slice(y, y + this.rowSpan) as number[]
  ...
  draw(lineNum: "top" | "bottom" | number, spanningCell?: number): string {
    ...
    if (!lineNum) {
      debug.info(
        `${this.y}-${this.x}: ${this.rowSpan - lineNum}x${this.colSpan} Cell ${content}`,
      );
    } else {
  ...
  drawTop(drawRight: boolean): string {
    ...
    if (this.cells) {
      ...
      content.push(
        repeat(this.chars[this.y === 0 ? "top" : "mid"], this.width),
      );
    }
    if (drawRight) {
  ...
  drawLine(
    ...
    if (this.x && this.x > 0 && spanningCell && this.cells && this.y !== null) {
      ...
      while (
        cellLeft instanceof ColSpanCell &&
        cellLeft.y !== null &&
        cellLeft.x !== null
      ) {
...
export class RowSpanCell {
  ...
  init(tableOptions: TableInstanceOptions): void {
    if (this.y === null || this.originalCell.y === null) {
      throw new Error(
        "RowSpanCell y and originalCell.y must be initialized before calling init",
      );
    }
    const y = this.y;
    ...
    this.offset = findDimension(
      tableOptions.rowHeights as number[],
      originalY,
      this.cellOffset,
    );
  }
