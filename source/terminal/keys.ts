...
/**
 * Check if input matches Ctrl+N (raw byte or Kitty protocol).
 * Ignores lock key bits.
 */
export function isCtrlN(data: string): boolean {
  return (
    data === RAW.CTRL_N ||
    data === Keys.CTRL_N ||
    matchesKittySequence(data, CODEPOINTS.n, MODIFIERS.ctrl)
  );
}
...
export function isArrowUp(data: string): boolean {
  return (
    data === "\x1b[A" || matchesKittySequence(data, ARROW_CODEPOINTS.up, 0)
  );
}
...
export function isArrowDown(data: string): boolean {
  return (
    data === "\x1b[B" || matchesKittySequence(data, ARROW_CODEPOINTS.down, 0)
  );
}
...
export function isArrowRight(data: string): boolean {
  return (
    data === "\x1b[C" || matchesKittySequence(data, ARROW_CODEPOINTS.right, 0)
  );
}
...
export function isArrowLeft(data: string): boolean {
  return (
    data === "\x1b[D" || matchesKittySequence(data, ARROW_CODEPOINTS.left, 0)
  );
}
