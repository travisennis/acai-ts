  ...
  return {
    [toolName]: {
      ...
      ask: async (
        ...
        // Check if the tool needs approval based on its metadata
        if (metadata.needsApproval === false) {
          return { approve: true };
        }

        // For security, default to requiring approval for dynamic tools
        // If toolExecutor is available, use it to ask the user
        return {
          approve: false,
          reason:
