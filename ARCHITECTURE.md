...
*   **`.acai/`**: Directory for acai's internal state, configuration, and temporary files.
    ...
    *   `rules/learned-rules.md`: Stores rules learned by acai based on user corrections and feedback.
...
*   **`source/`**: Contains all the source code for the acai application.
    ...
    *   `conversation-analyzer.ts`: Implements logic to analyze the conversation history between the user and acai, identifying user corrections to infer new, generalizable rules for improving acai's behavior. These learned rules are stored in `~/.acai/rules/learned-rules.md`.
    ...
    *   `prompts.ts`: Exports the `systemPrompt` function, which dynamically generates the main system prompt provided to the AI model, incorporating core principles, work standards, tool usage guidelines, project-specific rules (`AGENTS.md`), and environment information.
